...
export interface SpecularButtonProps extends Partial<SpecularEffectOptions> {
  children?: ReactNode;
  size?: ButtonSize;
  tint?: string;
  tintOpacity?: number;
...
/**
 * Headless specular-edge effect. Renders the animated rim light into `fxRef`,
 * sized and angled against the element referenced by `targetRef`. Kept
 * separate from the button markup so the same effect can be overlaid onto an
 * existing, differently-styled button without touching its classes.
 */
export function useSpecularEffect(
  targetRef: RefObject<HTMLElement | null>,
  fxRef: RefObject<HTMLElement | null>,
  options: SpecularEffectOptions
) {
  const propsRef = useRef<SpecularEffectOptions>(options);
  // Keep the latest options available to the rAF loop without restarting the
  // effect. Synced post-commit so we never mutate a ref during render.
  useEffect(() => {
