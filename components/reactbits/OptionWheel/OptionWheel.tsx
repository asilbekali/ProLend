...
const OptionWheel = ({
  ...
  // Keep the latest props/config available to the rAF loop and event handlers
  // without re-subscribing; synced post-commit so we never write refs in render.
  useEffect(() => {
  ...
  // Hold the latest runFrame so the loop can re-schedule itself without the
  // callback referencing its own binding during declaration.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // When not looping, release the wheel at the ends so the page can keep
      // scrolling once every option has been passed (down past the last, up
      // past the first) instead of trapping the scroll here.
      const atTop = !cfg.loop && delta < 0 && targetRef.current <= 0.0001;
      const atBottom = !cfg.loop && delta > 0 && targetRef.current >= cfg.count - 1 - 0.0001;
